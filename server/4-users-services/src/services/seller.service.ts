...
import { IOrderMessage, IRatingTypes, IReviewMessageDetails, ISellerDocument } from '@harish-x/jobber-helpers';
...
const getSellerById = async (sellerId: string): Promise<ISellerDocument | null> => {
  const seller: ISellerDocument | null = await SellerModel.findById(sellerId) as ISellerDocument;
  return seller;
};
