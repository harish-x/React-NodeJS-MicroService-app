...
import { IBuyerDocument, ISellerDocument, winstonLogger } from "@harish-x/jobber-helpers";
import { Logger } from "winston";
import { createConnection } from "@users/queues/connection";
import { Channel, ConsumeMessage, Replies } from "amqplib";
import { createBuyer, updateBuyerPurchesedGigsProp } from "@users/services/buyer.service";
import { getRandomSellers, updateSellerCancelledJobsProp, updateSellerCompletedJobsProp, updateSellerOngoingJobsProp, updateSellerReview, updateTotalGigsCount } from "@users/services/seller.service";
import { publishDirectMessage } from "./user.producer";


const log:Logger = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'UsersQueueProducer', 'debug');
    ...
    try {
        ...
        channel.consume(jobberQueue.queue, async (msg : ConsumeMessage | null) => {
            ...
            if(type === 'auth') {
                ...
                const buyer:IBuyerDocument = {username,email,profilePicture,country,purchasedGigs:[],createdAt} as IBuyerDocument;
                await createBuyer(buyer);
            }else{
                const {buyerId,purchasedGigs,type} = JSON.parse(msg!.content.toString());
                await updateBuyerPurchesedGigsProp(buyerId,purchasedGigs,type);
            }
            channel.ack(msg!); 
        });
    } catch (error) {
        log.log('error','users service consumeBuyerDirectMessage() method error:', error);
    }
}
...
const consumeSellerDirectMessage = async (channel:Channel): Promise<void> => {
    try {
        ...
        log.log('error','users service consumeSellerDirectMessage() method error:', error);
    }
}
    ...
    try {
        ...
        channel.consume(jobberQueue.queue, async (msg : ConsumeMessage | null) => {
           ...
           if(type === 'buyer-review'){
                ...
                await publishDirectMessage(channel,'jobber-update-gig','update-gig',JSON.stringify({type:'updateGig',gigReview:msg!.content.toString()}),'Message published to jobber-update-gig fanout exchange');
           }
            channel.ack(msg!);
        });
    } catch (error) {
        log.log('error','users service consumeReviewFanoutMessages() method error:', error);
    }
}
